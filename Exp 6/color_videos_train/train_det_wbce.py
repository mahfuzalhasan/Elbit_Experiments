...
def train(run_id, epoch, train_dataloader, model, model_classification, optimizer, criterion, criterion_classification, writer, use_cuda, lr_scheduler=None, lr_list= None):
    ...
    for i, (inputs, tubes, targets, label_masks, possible_targets) in enumerate(train_dataloader):
        ...
        print_order = False
        with torch.no_grad():
            outputs_numpy = get_bbox_map(i, print_order, inputs, model)  #b,f,h,w --> 6,16,448,800
        #print('outputs numpy: ',outputs_numpy.shape)
        labels = []
        ...
        for b in range(outputs_numpy.shape[0]):
            ...
            if i%40==0:
                ...
                for k in range(outputs_numpy_b.shape[0]):
                    ...
                    cv2.imwrite(os.path.join(save_batch,f'loc_mask_itr_{i}_clip_{b}_frame_{k}.png' ),mask_batch*255)
            ...
            for blob in blobs:
                ...
                overlap = 1 if overlap>=0.15 else 0            #only select those bbox that has big overlap with gt tube
                if overlap<1:
                    continue
                #print('batch: ',i, ' bbox:',bbox)
                bboxes.append(bbox)
        ...
        class_label = model_classification(inputs, localization_tube)
        #print('class label: ',class_label.size())
        # PLM
...
def validation(run_id, epoch, valid_dataloader, model, model_classification, criterion, criterion_classification, writer, use_cuda, lr_scheduler=None):
    ...
    with torch.no_grad():
        for i, (inputs, tubes, targets, label_masks, possible_targets) in enumerate(valid_dataloader):
            ...
            print_order = False
            outputs_numpy_b = get_bbox_map(i, print_order, inputs, model)  #b,f,h,w --> 1,16,448,800
            outputs_numpy_b = outputs_numpy_b[0]
            ########################################################localization tube extraction
            ...
            if len(bboxes)>0:
                for l_tube in bboxes: 
                    ...
                    class_label_b = model_classification(inputs, localization_tube_mask)
...
def train_locator( run_id, use_cuda ):
    ...
    model_classification = InceptionI3d(num_classes=157, in_channels=3)
    saved_model_file = "/home/c3-0/praveen/VIRAT/trained_models/i3d_model_rgb_charades.pt"
    if saved_model_file is not None:
        model_classification.load_state_dict(torch.load(saved_model_file),strict=False)
