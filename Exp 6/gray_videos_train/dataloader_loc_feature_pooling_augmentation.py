...
if __name__ == '__main__':
    ...
    val_dataset = MEVADataGenerator('validation', params.train_percent, params.train_scales, use_localization_alone = False, use_groundtruth_alone = False)
    start = time.time()
    dataloader = DataLoader(val_dataset, batch_size=1, shuffle=shuffle, num_workers=1, collate_fn=filter_none)
    epoch = 0
    ...
    for i, (inputs, seg_maps, targets, label_masks, possible_targets) in enumerate(dataloader):
        ...
        input_storage.append(inputs)
        label_storage.append(targets)
        
        if i %10 ==9:
            pickle.dump(input_storage, open('val_frames.pkl', 'wb'))
            pickle.dump(label_storage, open('val_labels.pkl', 'wb'))
            exit()
